...
def update_day(agent):
    ...
    if agent.health_condition=="to_be_recover":
        ...
        agent.model.infected -= 1 #Update amount infected at any given time
        agent.model.susceptible += 1
def get_completion_from_messages(message, temperature=0):
...
def get_completion_from_messages_json(message, temperature=0):
    client = OpenAI(api_key="",
                    base_url="")
    response = client.chat.completions.create(
        model='Qwen/Qwen2.5-7B-Instruct',
    ...
    # Convert the generated text content to JSON format
    response_json = {
        "response": generated_text
    }
    # Print the JSON format content
    json_str = json.dumps(response_json, ensure_ascii=False, indent=4)
